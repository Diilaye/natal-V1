...
const CartHeader = () => {
  ...
  const [defaultData, setDefaultData] = useState<CartHeaderDataType[]>(cartHeaderData);
  ...
  const handleValue = (data: CartHeaderDataType, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setDefaultData(defaultData.map((item) => (item.title === data.title ? { ...data, value: numericValue } : item)));
  };

  const decrementQty = (item: CartHeaderDataType) => {
    setDefaultData(defaultData.map((data) => (data.title === item.title ? { ...data, value: Math.max(0, data.value - 1) } : data)));
  };
  ...
  const orderTotal = defaultData.reduce((total, item) => total + (item.price * item.value), 0);
    ...
    <li className="custom-dropdown">
      ...
      <div className={`custom-menu cart-dropdown py-0 overflow-hidden ${show ? "show" : ""}`}>
        ...
        <ul className="pb-0">
          {defaultData.map((data, index) => (
            <li key={index}>
              <div className="d-flex">
                <img className="img-fluid b-r-5 me-3 img-60" src={`${ImagePath}/${data.image}`} alt="" />
                <div className="flex-grow-1">
                  <span className="f-w-600">{data.title}</span>
                  <div className="qty-box">
                    <InputGroup>
                      <span className="input-group-prepend" onClick={() => decrementQty(data)}>
                        <Button className="quantity-left-minus" type="button">
                          -
                        </Button>
                      </span>
                      <Input className="input-number" type="number" name="quantity" value={data.value} onChange={(e) => handleValue(data, e)} />
                      <span className="input-group-prepend" onClick={() => incrementQty(data)}>
                ...
                <div className="close-circle">
                  <a className="bg-danger" href={Href} onClick={() => handleDelete(data)}>
                    <X />
                  </a>
                </div>
              </div>
          ...
          <li className="total">
            <h6 className="mb-0">
              Order Total : <span className="f-w-600">${orderTotal.toFixed(2)}</span>
            </h6>
          </li>
          <li className="text-center">
            <Link className="d-block mb-3 view-cart f-w-700 text-primary" href={Href}>
              Go to your cart
            </Link>
            <Link className="btn btn-primary view-checkout text-white" href={Href}>
              Checkout
            </Link>
          </li>
        </ul>
